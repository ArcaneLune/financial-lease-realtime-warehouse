...
本项目模拟**金融租赁公司授信审批业务**。金融租赁行业的核心业务是授信审批：客户（承租人）向租赁公司申请授信，用于购买设备等资产后"以租代购"。这一业务有三个特点，决定了它对**实时数据**的强依赖：

1. **审批链路长、环节多**：一笔授信要经过风控初审 → 信审 → 一级/二级评审 → 项目评审会 → 总经理逐级审批，节点多、流转复杂，管理层必须实时掌握每个项目"卡在哪个环节"。
2. **资金体量大、周期长**：单笔授信动辄数百万至上亿，从申请到起租通常以月计，任何环节的延误都意味着资金占用和风险敞口。
3. **强监管、强风控**：金融监管要求授信全流程留痕、可追溯、准实时报送；欺诈识别、额度超限等风控场景需要秒级响应。

传统的 T+1 离线数仓（次日才能看到昨天数据）无法满足上述诉求，因此需要建设一套 **Flink + Doris 实时数仓**，把授信审批全链路数据从产生到可视化压缩到秒级。
...
| 授信转化漏斗无法实时跟踪 | 月底才出报表 | 10 秒窗口实时刷新 申请→批复→占用→签约→起租 转化漏斗 |
...
本项目模拟**金融租赁授信审批业务**：承租人（企业客户）向租赁公司申请授信，经多级审批后获得授信额度，随后完成授信占用、签订合同并起租（租出设备）。全流程围绕主表 `credit_facility`（授信申请）展开，配合多张流转表 / 明细表记录每个环节。
...
> **关键口径（本项目踩坑后沉淀的面试点）**：审批是否通过、授信是否占用、合同是否起租，必须看**流水记录 / 时间字段**，而不是主表 `status` 的当前值——因为状态会继续流转（如 `16` 会流转到 `19` 新增授信），只看当前值会漏数。
